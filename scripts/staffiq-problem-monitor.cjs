...
process.env.STAFFIQ_PROBLEM_MONITOR_INTERVAL_HOURS = String(monitorCadenceHours)
const ownerHeaders = {
  'X-Staffiq-User-Id': process.env.STAFFIQ_MONITOR_USER_ID || 'U001',
  'X-Staffiq-User-Email': process.env.STAFFIQ_MONITOR_USER_EMAIL || 'admin@iicocece.com',
  'X-Staffiq-User-Role': process.env.STAFFIQ_MONITOR_USER_ROLE || 'super_admin',
  'X-Staffiq-Owner': process.env.STAFFIQ_MONITOR_OWNER || 'ayodeji_falope',
}
...
async function main() {
  const endpoint = `${targetUrl}/api/staffiq-problem-reports?status=${encodeURIComponent(statusFilter)}`
  ...
  console.log(`Staffiq Codex problem monitor`)
