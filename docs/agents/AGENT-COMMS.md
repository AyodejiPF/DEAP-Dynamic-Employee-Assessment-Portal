...
**Date**: 2026-07-28
**Prompt Ref**: Chat: Version Tracker + Learning/Help content refresh | Date: 2026-07-28 | "Update the in-app Version Tracker (and any equivalent help/learning content) so every new and improved feature from recent sessions is properly documented inside the app itself, discoverable by Super Admin and users, not just sitting in a developer log."
**Task**: Read this log in full and cross-checked it against `git log --oneline -30` on `main`. Found the Version Tracker (`src/data/changeCatalog.ts`) stopped at VT-13, but eleven more real commits had landed on `main` since then without a matching catalogue entry: the Firestore legacy collection rename cutover, the AI hierarchy guard going live, the Version Tracker/Feature Parity tabs actually being mounted into `src/App.tsx`, the My Team manager view, the platform-level AI key pool, the per-tenant API Keys dashboard, a Flutterwave checkout safety gate, a hardcoded-identity-check cleanup, an admin cloud-save reliability fix, and a CI/test hardening pass. Added VT-14 through VT-23 for these, each sourced to a specific dated AGENT-COMMS entry or a specific git commit, plus a VT-24 entry documenting this session's own work (this catalogue's own append-only convention). Also checked StaffiQ for a Task-Pulse-style Learning Centre/Help/FAQ equivalent — one already exists (`HelpFaq` component + `helpKnowledgeItems` array in `src/App.tsx`, ~21,500 lines in) — and added three new entries there matching its existing `HelpContentItem` structure and tone: an AI privacy FAQ explaining the hierarchy guard, a My Team guide, and a Version Tracker (SuperAdmin only) guide.
**Files**:
- `src/data/changeCatalog.ts` — added VT-14 through VT-24 (11 new entries) at the top of `CHANGE_CATALOG_ENTRIES`, continuing the existing numbering/structure. No prior entries edited or removed.
- `src/App.tsx` — added three entries to `helpKnowledgeItems`: `help-ai-hierarchy-guard` (faq, audience `all`), `help-my-team-view` (guide, audience `all`), `help-version-tracker-superadmin` (guide, audience `admin`, following the same "(SuperAdmin only)" convention already used for `help-ai-usage-superadmin` and `help-codex-repair`). Deliberately did not add a Help Centre entry for the Firestore rename — it has no visible user-facing behaviour to explain.
- This log entry.
**Status**: completed
**Evidence**:
- Real repo state check first: `main` was at `a2eba64`; the working copy of this repo (the one with an open editor session) was 2 commits ahead on a separate branch `speed/staffiq-favicon-og-optimisation` (favicon/font work, uncommitted xlsx + 2 new doc files present) — deliberately did not touch that branch or its uncommitted files. Did all real work in a disposable `git worktree` checked out to `main` instead, to avoid any collision.
- `tsc --noEmit -p tsconfig.app.json` → 0 errors.
- `tsc --noEmit -p functions/tsconfig.json` → the same 4 pre-existing errors in `functions/src/billing/entitlementEndpoint.ts` documented by every prior session (confirmed this session touched zero files in `functions/`), nothing new.
- `vite build` → succeeded for real on this machine: 2324 modules transformed, 0 errors, only the pre-existing >500kB chunk-size warning.
- Committed to `main` as `a2e7227` and pushed: `git push origin main` → `a2eba64..a2e7227 main -> main`.
- Deployed for real via a detached Windows Scheduled Task (direct `firebase deploy` calls in this environment get killed when a tool call ends, same workaround documented by the 2026-07-26 entry above), polling a log file rather than `--debug`:
  - First combined `--only functions,hosting,firestore` attempt: hosting file upload completed but the release did not finalize before ~21 *pre-existing* functions (none touched by this session — apiKeys, aiProviderKeys/AI key pool, billing checkout/subscription, grants) failed with "Quota exceeded for total allowable CPU per project per region" on Cloud Run — a real, transient GCP quota limit from too many concurrent 2nd-gen function rollouts, not caused by this session's changes.
  - Re-ran `--only hosting` alone: `hosting[training-assessment-1c8ef]: release complete` / `Deploy complete!` — this carries this session's actual changes (the catalogue and Help Centre entries).
  - Re-ran `--only functions,firestore`: `Deploy complete!`, every previously-failed function now reported `Skipped (No changes detected)`, confirming the earlier partial attempt had in fact already succeeded for them.
- Live HTTP verification: `https://training-assessment-1c8ef.web.app` → 200, real StaffiQ title/meta. `https://training-assessment-1c8ef.web.app/robots.txt` → 200, real content (confirms VT-22's robots.txt is genuinely live). `https://staffiq.ng` → 200, unaffected marketing site.
- Did not attempt an authenticated in-app check (logging in as the test admin account and clicking through to Super Admin → Version Tracker) — build, typecheck, deploy logs, and unauthenticated HTTP checks were the real verification done this session; flagging this honestly as the one thing not verified live behind login.
**Notes**: The Cloud Run CPU quota ceiling hit mid-deploy is worth a proactive look by whoever next touches `functions/` — with apiKeys, aiProviderKeys, billing, and grants all now separate function groups, a single `firebase deploy --only functions` for this project may keep tripping that quota as more functions are added; deploying functions in smaller batches (or requesting a quota increase) would avoid the two-pass workaround used here. Cleaned up: deleted all three scheduled tasks (`StaffiQDeployVT`, `StaffiQDeployHostingVT`, `StaffiQDeployFn2VT`) and removed the temporary git worktrees after finishing, so nothing was left running or checked out unnecessarily.
